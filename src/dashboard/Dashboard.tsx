...
import { Customer, Order, Review, Invoice } from '../types';
...
const Dashboard: FC = () => {
    ...
    const fetchOrders = useCallback(async () => {
        ...
        const { data: recentOrders } = await dataProvider.getList<Order>(
            'commands',
            {
                filter: { add_time_gte: aMonthAgo.toISOString() },
        ...
        const revenue = recentInvoices.reduce((accumulator, invoice) => {
            accumulator += invoice.total;
            return accumulator;
        }, 0);
